#' Export the a 6000 x 6000 px jpeg or png image
...
square <- function(filename = "plot", plot, filetype = c("jpeg", "png"), dpi = 300) {
  if(filetype == "jpeg") {
    dev.print(jpeg, file = filename, width = 6000, height = 6000, res = dpi, units = "px")
    print(plot)
    dev.off()
  } else if (filetype == "png") {
    dev.print(png, file = filename, width = 6000, height = 6000, res = dpi, units = "px")
    print(plot)
    dev.off()
  } else {
