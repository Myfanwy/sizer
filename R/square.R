...
square <- function(filename, plot, filetype = c("jpeg", "png"), dpi = 300) {
  if(filetype == "jpeg") {
    jpeg(file = filename, width = 6000, height = 6000, quality = dpi, units = "px")
    plot
    dev.off()
  } else if (filetype == "png") {
    png(file = filename, width = 6000, height = 6000, res = dpi, units = "px")
    print(plot)
    dev.off()
  } else {
